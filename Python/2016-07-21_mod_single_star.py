...
def tov_RHS(P, M, r, dM):
    ...
    print "args=",P, M, r, dM
    print "vars=",rho, drhoP, ddrhoPP, g_c2
    print "lambda1=",lambda1
    print "fR=",fR
    print "constants=",alphaQ,Rp,1/Rp
    print "sigma1=",sigma1,fR/2.0 , math.sqrt(2)*math.sqrt(alphaQ), \
        math.sqrt(-8*G*Pi*(P + rho) + lambda1**2),math.sqrt(2)*math.sqrt(alphaQ)* \
        math.sqrt(-8*G*Pi*(P + rho) + lambda1**2)
    print "sigma2=",sigma2
    print "f=",f
...
#    print "tautt",tau_tt
    ...
    print "Sqrt(2)*lambda",lambda1*math.sqrt(2)
...
#---dP------------------------------------------------------------------------
    ...
    ddlambdaPP = ((math.sqrt(alphaQ)*(96*ddrhoPP*G*Pi + (((-32*(1 + drhoP)*G*Pi)/alphaQ + 
        16*(-3 + drhoP)*G*Pi*(fR/alphaQ + 8*G*Pi*(-3*P + rho)))**2*s)/
        ((-32*G*Pi*(P + rho))/alphaQ + (fR/alphaQ + 8*G*Pi*(-3*P + rho))**2)**1.5 - 
        (32*G*Pi*(72*alphaQ*G*Pi - 48*alphaQ*drhoP*G*Pi + 8*alphaQ*drhoP**2*G*Pi + 
        ddrhoPP*(-2 + fR - 24*alphaQ*G*P*Pi + 8*alphaQ*G*Pi*rho))*s)/
        (alphaQ*math.sqrt((-32*G*Pi*(P + rho))/alphaQ + (fR/alphaQ + 
        8*G*Pi*(-3*P + rho))**2))))/(16.*math.sqrt(2)))
