...
global ggrav
ggrav = Dec(6.673*10**-8)
G = ggrav
global clite
clite = Dec(2.99792458*10**10)
global g_c2
g_c2 = ggrav/clite**2
global msun
msun = Dec(1.98892*10**33)
global pi
pi = Dec(math.pi)
Pi = Dec(math.pi)
#global gamma
...
#eosk = 1.98183*10**-6
global min_press
min_press = Dec(1*10**-9)
global delta
delta = Dec(0.23)
rho_center = Dec(2.0*(10**14))
alphaQ = Dec(1.0)
Rp = Dec((1.0/1.6)**33)
a = Dec(1/2)
s=Dec(1)
...
rhoFromP = interpolate.InterpolatedUnivariateSpline(p_EOS, rho_EOS, k=2)
pFromRho = interpolate.InterpolatedUnivariateSpline(rho_EOS, p_EOS, k=2)
dRhoDP = rhoFromP.derivative()
ddRhoDP = rhoFromP.derivative(2)
...
def tov_integrate( roh_c ):
    ...
    press_now = Dec.from_float(press)
    m_now = Dec(0.0)
    count = 0
    m_now = Dec(0.0)
    rad = Dec(0.0)
...
def tov_RHS(P, M, r, dM):
    ...
    rho = Dec.from_float(rhoFromP(P))*g_c2/(clite**2)
    drhoP = Dec(dRhoDP(P))
    ddrhoPP = Dec(ddRhoDP(P))
    M=M*g_c2
